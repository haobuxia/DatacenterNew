...
//        cc.add(Calendar.MONTH,-1);
    ...
    public static String getTimeBetweenTwo(String checkStart,String checkEnd) {
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date checkStartDate = null;
        Date checkEndDate = null;
        if (checkStart!=null&&checkEnd!=null) {
            try {
                checkStartDate = simpleDateFormat.parse(checkStart);
                checkEndDate = simpleDateFormat.parse(checkEnd);
            } catch (ParseException e) {
                e.printStackTrace();
            }
            long start = checkStartDate.getTime();
            long end = checkEndDate.getTime();
            if (start <= end) {
    ...
    public static void main(String[] args) {
        String getmin = getTimeStrBySecond(600);
