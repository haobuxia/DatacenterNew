...
public class ExcelUtil {
    ...
    private static final int DataFormat = 176;
     ...
     */
    public static <T> List<Map> importExcel(Class<T> clazz, InputStream inputStream,
            ...
            while (rowIterator.hasNext()) {
                ...
                if (clazz == Map.class) {
                    ...
                    for (String k : titleMap.keySet()) {
                        ...
                        if (cell.getCellStyle().getDataFormat()==DataFormat){
                            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                            double numericCellValue = cell.getNumericCellValue();
                            Date javaDate = DateUtil.getJavaDate(numericCellValue);
                            String format = simpleDateFormat.format(javaDate);
                            if (format.equalsIgnoreCase("1899-12-31 00:00:00")){
                                map.put(k,"");
                            }else {
                                map.put(k,format);
                            }
    ...
    public static <T> List<Map> importExcel2(Class<T> clazz, InputStream inputStream,
                                            String pattern, ExcelLogs logs, Integer... arrayCount) {
        Workbook workBook;
        try {
            workBook = WorkbookFactory.create(inputStream);
