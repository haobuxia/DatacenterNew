...
//        resultMap.put("modelName", param.get("modelName"));
    ...
    public ResponseVo onetime(Map<String, String> param) {
        ...
        DSParamDsBuilder dsParamDsBuilder = new DSParamDsBuilder(62);
        try {
            //{"condition":{"endTime":"d","startTime":"d","stationName":"d","modelName":"d","typeName":"d"},"dataObjectId":62,
        ...
        com.tianyi.datacenter.feign.common.vo.ResponseVo retrieveCarOneTime = dataCenterFeignService.retrieve(dsParamDsBuilder.build());
        if (retrieveCarOneTime.isSuccess() && retrieveCarOneTime.getMessage() == null) {
            List<Map<String, String>> rtnDataCarOneTime = (List<Map<String, String>>) retrieveCarOneTime.getData().get("rtnData");
            if (rtnDataCarOneTime.size() > 0 && rtnDataCarOneTime != null) {
                //所有工单ID
                for (Map<String, String> stringStringMap : rtnDataCarOneTime) {
                    String cid = stringStringMap.get("cid");
                    //{"condition":[{"key":"checkorderId","condition":"equals","value":"s"}],"dataObjectId":17,"pageInfo":{"page":1,"count":0,"pageSize":100,"total":180}}
                    DSParamBuilder dsParamBuilder = new DSParamBuilder(17);
                    dsParamBuilder.buildCondition("checkorderId", "equals", cid);
                    com.tianyi.datacenter.feign.common.vo.ResponseVo retrieveCheckOrderItem = dataCenterFeignService.retrieve(dsParamBuilder.build());
                    if (retrieveCheckOrderItem.isSuccess() && retrieveCheckOrderItem.getMessage() == null) {
                        List<Map<String, Object>> rtnDataCheckOrderItem = (List<Map<String, Object>>) retrieveCheckOrderItem.getData().get("rtnData");
                        //获取所有检查项目
                        if (rtnDataCheckOrderItem.size() > 0 && rtnDataCheckOrderItem != null) {
                            for (Map<String, Object> map : rtnDataCheckOrderItem) {
                                map.put("createTime",stringStringMap.get("createTime"));
                                CheckOrderItemListTotal.add(map);
//                                System.out.println((String) map.get("cid"));
                ...
                for (int i = 0; i < date.size(); i++) {
                    ...
                    float pass = 0;
                    float problem = 0;
                    for (Map<String, Object> stringStringMap : CheckOrderItemListTotal) {
//                        System.out.println(stringStringMap.get("checkEnd") + "   " + stringStringMap.get("cid") + "    " + stringStringMap.get("result"));
                        if ((String) stringStringMap.get("result") != null && stringStringMap.get("result") != "") {
                            String checkEnd = (String) stringStringMap.get("checkEnd");
//                            String checkStart = (String) stringStringMap.get("checkStart");
                            String checkStart = (String)stringStringMap.get("createTime");
...
//                                    System.out.println((String) stringStringMap.get("result"));
                                    if (((String) stringStringMap.get("result")).equalsIgnoreCase("1")) {
                                        pass++;
                                    } else {
                                        problem++;
                                    }
            ...
            return ResponseVo.success(resultList,"查询数据为空!");
        }
        ...
        for (Date date1 : date) {
            Map<String, Object> resultMap = new HashMap<>();
            String parseDate = TimeUtil.parse(date1);
            float resultPass = 0;
            float resultProblem = 0;
            for (Map map2 : numList) {
                String date2 = (String) map2.get("date");
                if (parseDate.equalsIgnoreCase(date2)) {
                    float pass2 = (float) map2.get("pass");
                    resultPass += resultPass + pass2;

                    float problem2 = (float) map2.get("problem");
                    resultProblem += resultProblem + problem2;
                }
            }
            resultMap.put("pass", resultPass);
            resultMap.put("problem", resultProblem);
            resultMap.put("date", parseDate);
            resultList.add(resultMap);
        }
        //添加合格率
                    ...
                    if (retrieveCheckOrderItem.isSuccess() && retrieveCheckOrderItem.getMessage() == null) {
                        ...
                        if (rtnDataCheckOrderItem.size() > 0 && rtnDataCheckOrderItem != null) {
                            for (Map<String, Object> map : rtnDataCheckOrderItem) {
                                map.put("createTime",stringStringMap.get("createTime"));
                                CheckOrderItemListTotal.add(map);
//                                System.out.println((String) map.get("cid")+"   "+stringStringMap.get("createTime"));
                            }
                        }
...
//                            String checkStart = (String) stringStringMap.get("checkStart");
        ...
        for (Date date1 : date) {
            Map<String, Object> resultMap = new HashMap<>();
            String parseDate = TimeUtil.parseDay(date1);
            float resultPass = 0;
            float resultProblem = 0;
            for (Map map2 : numList) {
                String date2 = (String) map2.get("date");
