...
public class CarArchivesController {
    ...
    public ResponseVo search(@RequestBody Map<String, Object> param) {
        ...
        condition.put("jihao",(String) condition.get("carId"));
...
//                                        returnMap.put("outEnd", rtnDataTime.get(0).get("onEnd"));
                            ...
                            DSParamDsBuilder dsParamDsBuilderEndTime = new DSParamDsBuilder(71);
                            dsParamDsBuilderEndTime.buildCondition("stationName",station);
                            dsParamDsBuilderEndTime.buildCondition("jihao",rtnData.get("cid"));
                            dsParamDsBuilderEndTime.buildCondition("modelName",rtnData.get("modelName"));
                            com.tianyi.datacenter.feign.common.vo.ResponseVo responseVoEndTime = dataCenterFeignService.retrieve(dsParamDsBuilderEndTime.build());
                            if (responseVoEndTime.getData().get("rtnData") != null) {
                                List<Map> rtnDataTime = (List<Map>) responseVoEndTime.getData().get("rtnData");
                                if (rtnDataTime.size() != 0) {
                                    if (station.equals("台上检查")) {
//                                        returnMap.put("onStart", rtnDataTime.get(0).get("onStart"));
                                        returnMap.put("onEnd", rtnDataTime.get(0).get("onEnd"));
